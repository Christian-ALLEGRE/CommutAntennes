...
void Arduino_GFX_Button::initButton(Arduino_GFX *gfx,
					  int16_t x, int16_t y, 
					  uint8_t w, uint8_t h, 
...
void Arduino_GFX_Button::setColor(int fill, int textcolor)
{
  _fillcolor = fill;
  _textcolor = textcolor;
  drawButton();
 ...
 void Arduino_GFX_Button::drawButton(boolean inverted) {
...
boolean Arduino_GFX_Button::contains(int16_t x, int16_t y) {
   if ((x < (_x - _w/2)) || (x > (_x + _w/2))) return false;
   if ((y < (_y - _h)) || (y > (_y + _h/2))) return false;
   return true;
 }

boolean Arduino_GFX_Button::containsH(int16_t x, int16_t y) {
   if ((y < (_x - _w/2)) || (y > (_x + _w/2))) return false;
   if ((x < (_y - _h)) || (x > (_y + _h/2))) return false;
   return true;
 }


 void Arduino_GFX_Button::press(boolean p) {
   laststate = currstate;
   currstate = p;
 }
 
 boolean Arduino_GFX_Button::isPressed() { return currstate; }
 boolean Arduino_GFX_Button::justPressed() { return (currstate && !laststate); }
 boolean Arduino_GFX_Button::justReleased() { return (!currstate && laststate);}
